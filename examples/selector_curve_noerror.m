...
param.name='BeH2'; %molecule name
param.transformation_type='jw red'; %hamiltonian construction method
param.electron_number=4; %number of valence electrons
...
%plotting data
figure(1);
plot(distance_axis,pseudo_potential_mean_axis,distance_axis,potential_noerror_mean_axis);
title('Potential mean');
xlabel('distance Angstroms');
ylabel('Energy [Hartrees]');
legend('pseudo','no error');

figure(2);
plot(distance_axis,pseudo_potential_std_axis,distance_axis,potential_noerror_std_axis);
title('Potential std');
xlabel('distance Angstroms');
ylabel('Energy [Hartrees]');
legend('pseudo','no error');
...
figure(3);
plot(distance_axis,difference_potential_noerror_mean_axis);
title('difference Potential mean');
xlabel('distance Angstroms');
ylabel('Energy [Hartrees]');

figure(4);
plot(distance_axis,difference_potential_noerror_std_axis);
title('difference Potential std');
xlabel('distance Angstroms');
ylabel('Energy [Hartrees]');

figure(5);
plot(distance_axis,fidelity_noerror_mean_axis);
title('Fidelity mean');
xlabel('distance Angstroms');
ylabel('Fidelity');

figure(6);
plot(distance_axis,fidelity_noerror_std_axis);
title('Fidelity std');
xlabel('distance Angstroms');
ylabel('Fidelity');